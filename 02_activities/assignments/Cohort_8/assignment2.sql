...
SELECT
product_name || ', ' || coalesce(product_size,'') || ' (' || coalesce(product_qty_type,'unit')|| ')' as 'product list (name, size, qty type)'
FROM product;
...
SELECT DISTINCT customer_id, market_date, visit_order AS most_recent_visit
...
DROP TABLE IF EXISTS temp.sales_per_date;
CREATE TABLE temp.sales_per_date AS
SELECT market_date,
...
CREATE TABLE temp.sales_ranking AS
...
JOIN vendor_inventory AS vi
  ON vp.vendor_id = vi.vendor_id
  AND vp.product_id = vi.product_id
JOIN vendor AS v
...
WHERE snapshot_timestamp='2025-11-25 00:39:28';
...
SET current_quantity = COALESCE (
 ( SELECT quantity
   FROM
    ...
    WHERE market_date_rank = 1 AND Last_qty.product_id=product_units.product_id)
	, 0);